...
#define N 1024
      ...
      CONTAINS
        INTEGER FUNCTION test_defaultmap_on()
          ...
          DO x = 1, N
            IF (char_array(x) .ne. scalar_char) THEN
              errors = errors + 1
            END IF
            IF (byte_array(x) .ne. scalar_byte) THEN
              errors = errors + 1
            END IF
            IF (short_array(x) .ne. scalar_short) THEN
              errors = errors + 1
            END IF
            IF (int_array(x) .ne. scalar_int) THEN
              errors = errors + 1
            END IF
            IF (long_int_array(x) .ne. scalar_long_int) THEN
              errors = errors + 1
            END IF
            IF (abs(float_array(x) - scalar_float) .gt. .00001) THEN
              errors = errors + 1
            END IF
            IF (abs(double_array(x) - scalar_double) .gt. .0000000001) THEN
              errors = errors + 1
            END IF
            IF (logical_array(x) .neqv. scalar_logical) THEN
              errors = errors + 1
            END IF
            IF (logical_kind4_array(x) .neqv. scalar_logical_kind4) THEN
              errors = errors + 1
            END IF
            IF (logical_kind8_array(x) .neqv. scalar_logical_kind8) THEN
              errors = errors + 1
            END IF
            IF (abs(REAL(complex_array(x)) - REAL(scalar_complex)) .gt. .00001)&
                & THEN
              errors = errors + 1
            END IF
            IF (abs(IMAG(complex_array(x)) - IMAG(scalar_complex)) .gt. .00001)&
                & THEN
              errors = errors + 1
            END IF
            IF (abs(REAL(double_complex_array(x)) - REAL(scalar_double_complex&
                &)) .gt. .0000000001) THEN
              errors = errors + 1
            END IF
            IF (abs(IMAG(double_complex_array(x)) - IMAG(scalar_double_complex&
                &)) .gt. .0000000001) THEN
              errors = errors + 1
            END IF
          END DO
