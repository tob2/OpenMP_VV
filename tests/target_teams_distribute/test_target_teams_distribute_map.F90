...
#define N 1024
      ...
      PROGRAM test_target_teams_distribute_map
        ...
        OMPVV_TEST_VERBOSE(test_from() .ne. 0)
        OMPVV_TEST_VERBOSE(test_alloc() .ne. 0)
        OMPVV_TEST_VERBOSE(test_tofrom() .ne. 0)

        OMPVV_REPORT_AND_RETURN()
      CONTAINS
        INTEGER FUNCTION test_to()
          INTEGER:: errors, x
          ...
          test_to = errors
        END FUNCTION test_to

        INTEGER FUNCTION test_from()
          INTEGER:: erorrs, x
          INTEGER,DIMENSION(N):: a, b
          ...
          !$omp target teams distribute map(from: a(1:N), b(1:N))
          DO x = 1, N
            b(x) = a(x)
          END DO
          ...
          test_from = errors
        END FUNCTION test_from

        INTEGER FUNCTION test_alloc()
          INTEGER:: errors, x
